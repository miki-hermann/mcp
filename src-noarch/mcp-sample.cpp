...
#include "mcp-basics.hpp"	// for split
...
void read_arg (int argc, char *argv[]) {
  ...
  while (argument < argc) {
	...
	cerr << "+++ no population option selected, revert to default" << arg << endl;
    } else if (arg == "--quiet"
	       || arg == "-q") {
      quiet = true;
	...
	cerr << "+++ no confidence interval selected, revert to default" << arg << endl;
    } else if (arg == "--bound"
	       || arg == "--error"
	       || arg == "-B"
	       || arg == "-b") {
	...
	cerr << "+++ no error bound selected, revert to default" << arg << endl;
	...
	cerr << "+++ no cardinality selected, revert to default" << arg << endl;
	...
	cerr << "+++ no proportion selected, revert to default" << arg << endl;
    } else if (arg == "--concept"
	       || arg == "-c") {
      if (argument < argc-1) {
	concept_column = argv[++argument];
      } else
	cerr << "+++ no concept column selected" << arg << endl;
    } else if (arg == "--big"
	       || arg == "--BIG") {
      big = true;
    } else
...
int main(int argc, char **argv) {
  ...
  if (population == absolute) {
	...
	+ (big ? " --big" : "")
	// + " --big"
	+ " --population absolute -q -# " + to_string(section)
	+ " -o " + output;
