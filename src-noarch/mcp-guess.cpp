...
size_t    ENUM_MAX   = 20;		// 50 80 200 20
const string QMARK   = "?";

enum item : short {UNDEF = 0, INT = 1, FLOAT = 2, STRING = 3};
...
void read_arg (int argc, char *argv[]) {	// reads the input parameters
  ...
  while (argument < argc) {
	...
	cerr << "+++ no attribute names file selected" << endl;
    } else if (arg == "-e"
	       || arg == "--enum") {
      if (argument < argc-1) {
	try {
	ENUM_MAX = stoul(argv[++argument]);
	} catch (invalid_argument err) {
	  cerr << "+++ " << argv[argument]
	       << " is not a valid maximum enumeration size"
	       << endl;
	  exit(2);
	}
      } else
	cerr << "+++ no maximal enumeration size selected, revert to default" << endl;
    // } else if (arg == "-r"
    // 	       || arg == "--ratio") {
